...
pub fn apply_patches_from_dif(
    dif_path: &Path,
    exe_path: &Path,
    log: Arc<Mutex<String>>,
) -> std::io::Result<()> {
    let patches = parse_dif_file(dif_path, log.clone())?;
    let backup_path = exe_path.with_extension(format!(
        "{}bk",
        exe_path.extension().unwrap_or_default().to_string_lossy()
    ));
...
pub fn parse_dif_file(
    dif_path: &Path,
    log: Arc<Mutex<String>>,
) -> std::io::Result<Vec<(u64, u8, u8)>> {
    ...
    for (index, line) in reader.lines().enumerate() {
        ...
        if line.starts_with("This difference file was") {
            continue;
        ...
        let address = u64::from_str_radix(&line_parts[0][2..], 16).map_err(|e| {
            log_lock.push_str(&format!(
                "Invalid address format at line {}: {}\n",
                index + 1,
                e
            ));
            std::io::Error::new(
        ...
        let old_byte = u8::from_str_radix(bytes[0], 16).map_err(|e| {
            log_lock.push_str(&format!(
                "Invalid old byte format at line {}: {}\n",
                index + 1,
                e
            ));
            std::io::Error::new(
        ...
        let new_byte = u8::from_str_radix(bytes[1], 16).map_err(|e| {
            log_lock.push_str(&format!(
                "Invalid new byte format at line {}: {}\n",
                index + 1,
                e
            ));
            std::io::Error::new(
